...
| **Admin** | - System Developer <br> - Provider [^1]  | - Developers of the OpenMRS have additional access to change fundamental structure of the database model <br> - All users with the 'Provider' role will appear as options in the default Infopath  | - Has all roles and privileges |
...
2. The clerk chooses the type of information to edit.
3. The clerk changes the information.
4. The clerk saves the form.
5. The system responds with confirmation of the changes.
...
4. The clerk chooses active status for the condition.
[A2: Choose inactive status]
...
1. Nurse searches for the patient in capture vitals app (either by name or patientId). 
[A1: Find active patients in active list]
2. The nurse chooses a patient to record vitals. 
...
2. Nurse searches for the patient (either by name or patientId).
3. Nurse can view the chosen patient's appointment schedules from the patient dashboard.
...
1. Doctor selects **Active Visits**
[A1: The patient has not been checked-in]
3. Doctor searches for the patient
4. On the chosen patient's dashboard, doctor admits/tranfers/discharges the patient. 
[A2: Cannot transfer/discharge patient]
6. Doctor may change **Admission Date** or **Admitted By** (the person performs this task)
7. Doctor saves the visit notes.
8. The system responds with confirmation of admission entered.
9. The system adds the admission and location on the Patient Dashboard.
...
3. Scheduler chooses the time block prefered to schedule a new appointment. 
[A1: No time slot available]
